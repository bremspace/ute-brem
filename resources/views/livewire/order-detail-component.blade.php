...
@endphp
<div x-data="{ expiresAt: new Date('{{ $order?->payment_expires_at?->toIso8601String() ?? now()->addHours(24)->toIso8601String() }}').getTime(), hours: '00', minutes: '00', seconds: '00', isExpired: false, showToast: false, toastMessage: '', init() { this.tick(); setInterval(() => this.tick(), 1000); }, tick() { const diff = this.expiresAt - Date.now(); if (diff <= 0) { this.hours = '00'; this.minutes = '00'; this.seconds = '00'; this.isExpired = true; this.toastMessage = 'Waktu pembayaran habis. Silakan buat pesanan baru.'; this.showToast = true; setTimeout(() => this.showToast = false, 2600); return; } const h = Math.floor(diff / 3600000); const m = Math.floor((diff % 3600000) / 60000); const s = Math.floor((diff % 60000) / 1000); this.hours = h < 10 ? '0' + h : '' + h; this.minutes = m < 10 ? '0' + m : '' + m; this.seconds = s < 10 ? '0' + s : '' + s; }, copy(text) { if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(text).then(() => { this.toastMessage = 'Nomor berhasil disalin'; this.showToast = true; setTimeout(() => this.showToast = false, 2600); }); } } }" x-init="init()" class="shop-container">
    {{-- Local icon sprite --}}
    ...
    @if ($order)
        ...
        <div class="countdown" data-expires="{{ $order->payment_expires_at?->toIso8601String() ?? now()->addHours(24)->toIso8601String() }}">
            <div class="countdown-box">
                <div class="countdown-num" x-text="hours">--</div>
                <div class="countdown-unit">Jam</div>
            </div>
            <div class="countdown-box">
                <div class="countdown-num" x-text="minutes">--</div>
                <div class="countdown-unit">Menit</div>
            </div>
            <div class="countdown-box">
                <div class="countdown-num" x-text="seconds">--</div>
                ...
                <div class="payment-box">
                    <div class="payment-box-title">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <use href="#c-icon-{{ $order->payment_method_name === 'Virtual Account' ? 'landmark' : ($order->payment_method_name === 'QRIS' ? 'qr' : ($order->payment_method_name === 'Kartu Kredit' ? 'credit-card' : 'wallet')) }}"/>
                        </svg>
                    ...
                    @if ($order->payment_method === 'va')
                        <div class="payment-va">
                            <div>
                                ...
                                <div class="payment-va-number">{{ $order->payment_reference ?: '8800 1234 5678 9012' }}</div>
                            </div>
        <button type="button" class="payment-copy-btn js-copy" data-copy="{{ $order->payment_reference ?: '8800123456789012' }}" @click="copy('{{ $order->payment_reference ?: '8800123456789012' }}')">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="true"><use href="#c-icon-copy"/></svg>
            Salin
        </button>
                        </div>
                    ...
                    @elseif ($order->payment_method === 'qris')
                        <div class="payment-qr">
                            {{-- Pseudo-QR determined by order code --}}
                            @php
                            $qrSize = 21;
                            $qrModules = [];
                    ...
                    @else
                        <ol class="payment-steps">
                            <li>Buka aplikasi {{ $order->payment_method_name }} di ponsel kamu.</li>
                ...
                <div class="order-meta">
                    ...
                    <div class="order-meta-item">
                        ...
                        <div class="order-meta-value">{{ $order->status_label }}</div>
                    </div>
                    <div class="order-meta-item">
