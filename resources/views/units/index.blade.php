    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex gap-2">
                ...
                @if(auth()->user()->hasPermission('master.products.create'))<a href="{{ route('units.create') }}" class="btn btn-primary"><i class="bx bx-plus me-1"></i>Tambah Satuan</a>@endif
            </div>
        </div>
        <div class="card-body">
            <div class="table-responsive text-nowrap">
                <table id="units-table" class="table table-striped">
                    <thead><tr><th>ID</th><th>Nama</th><th>Kode</th><th>Status</th><th>Aksi</th></tr></thead>
                </table>
            </div>
        </div>
    </div>
...
@push('scripts')
<script>
$(function(){ $('#units-table').DataTable({processing:true,serverSide:true,ajax:'{{ route('units.data') }}',columns:[{data:'id',name:'id'},{data:'name',name:'name'},{data:'code',name:'code'},{data:'status_badge',name:'is_active',orderable:false,searchable:false},{data:'action',name:'action',orderable:false,searchable:false}],dom:'lBfrtip',buttons:['copy','csv','excel','pdf','print','colvis']});});
</script>
@endpush
